...
describe('Unit test for auth', () => {
    ...
    test('doTokenAuth: when user is not loggedIn & getAuthToken have response', async () => {
        ...
        expect(authService.fetchAuthService).toHaveBeenCalledWith(thoughtSpotHost, username, 'authToken2');
    });
    ...
    test('doTokenAuth: when user is not loggedIn & fetchAuthPostService failed than fetchAuthService should call', async () => {
        ...
        jest.spyOn(authService, 'fetchAuthPostService').mockImplementation(() => Promise.reject({
            status: 500,
        }));
        jest.spyOn(authService, 'fetchAuthService').mockImplementation(() => Promise.resolve({
            status: 200,
        ...
        expect(authService.fetchAuthPostService).toHaveBeenCalledWith(thoughtSpotHost, username, 'authToken2');
        expect(authService.fetchAuthService).toHaveBeenCalledWith(thoughtSpotHost, username, 'authToken2');
    });
    ...
    describe('doSamlAuth', () => {
        ...
        it('should emit SAML_POPUP_CLOSED_NO_AUTH when popup window is closed', async () => {
            ...
            authInstance.samlCompletionPromise = Promise.resolve(false);
        ...
        it('when user is not loggedIn, in config noRedirect is true and isAtSSORedirectUrl is false', async () => {
            ...
            authInstance.samlCompletionPromise = Promise.resolve();
            global.window.open = jest.fn();
            jest.spyOn(tokenAuthService, 'isActiveService')
                .mockReturnValueOnce(Promise.resolve(false))
                .mockReturnValueOnce(Promise.resolve(true));
        ...
        it('should set loggedInStatus from cachedAuthToken without calling isLoggedIn again after popup flow', async () => {
            ...
            jest.spyOn(tokenAuthService, 'isActiveService')
                .mockReturnValueOnce(Promise.resolve(false));
            await authInstance.doSamlAuth({ ...exports.embedConfig.doSamlAuthNoRedirect });
            expect(authInstance.loggedInStatus).toBe(true);
        ...
        it('should store decoded accessToken in window when SAMLComplete event includes accessToken', async () => {
            ...
            authInstance.samlCompletionPromise = null;
            ...
            jest.spyOn(tokenAuthService, 'isActiveService')
                .mockReturnValueOnce(Promise.resolve(false));
            const authPromise = authInstance.doSamlAuth({ ...exports.embedConfig.doSamlAuthNoRedirect });
            await new Promise((resolve) => setTimeout(resolve, 0));
            expect(capturedMessageHandler).not.toBeNull();
            const accessToken = 'my-access-token';
            capturedMessageHandler({
                data: {
                    type: types_1.EmbedEvent.SAMLComplete,
                    accessToken: encodeURIComponent(accessToken),
                },
                source: { close: jest.fn() },
            });
            await authPromise;
            expect(checkReleaseVersionInBetaInstance.getValueFromWindow('cachedAuthToken')).toBe(accessToken);
            expect(authInstance.loggedInStatus).toBe(true);
        });
    });
    describe('doOIDCAuth', () => {
    ...
    it('authenticate: when authType is AuthServer', async () => {
        jest.spyOn(authInstance, 'doTokenAuth').mockReturnValue(Promise.resolve(true));
    ...
    it('authenticate: when authType is AuthServerCookieless', async () => {
        jest.spyOn(authInstance, 'doCookielessTokenAuth').mockReturnValue(Promise.resolve(true));
    ...
    it('getSessionDetails returns the correct details given sessionInfo', async () => {
        ...
        const devMockResponse = {
            ...exports.mockSessionInfoApiResponse,
            configInfo: {
                ...exports.mockSessionInfoApiResponse.configInfo,
                mixpanelConfig: {
                    ...exports.mockSessionInfoApiResponse.configInfo.mixpanelConfig,
                    production: false,
                },
            },
        };
