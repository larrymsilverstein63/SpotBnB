...
describe('unit test for utils', () => {
    ...
    describe('getRedirectURL', () => {
        ...
        test('Should return correct value when path is set', () => {
            expect((0, utils_1.getRedirectUrl)('http://myhost:3000/', 'hashFrag', '/bar')).toBe('http://localhost/bar#?tsSSOMarker=hashFrag');
            expect((0, utils_1.getRedirectUrl)('http://myhost:3000/#/foo', 'hashFrag', '#/bar')).toBe('http://localhost/#/bar?tsSSOMarker=hashFrag');
        });
    });
...
describe('Fullscreen Utility Functions', () => {
    ...
    describe('handlePresentEvent', () => {
        ...
        it('should catch error and log warning when requestFullscreen throws (covers line 506)', async () => {
            ...
            mockIframe.requestFullscreen.mockRejectedValue(new Error('permission denied'));
            await (0, utils_1.handlePresentEvent)(mockIframe);
            expect(logger_1.logger.warn).toHaveBeenCalledWith(expect.stringContaining('Failed to enter fullscreen'), expect.any(Error));
        });
...
describe('calculateVisibleElementData', () => {
    ...
    it('should calculate data for fully visible element', () => {
        ...
        const result = (0, utils_1.calculateVisibleElementData)(mockElement, true);
    ...
    it('should return zero dimensions when element is missing', () => {
        expect((0, utils_1.calculateVisibleElementData)(null)).toEqual({
            top: 0,
            height: 0,
            left: 0,
            width: 0,
...
describe('getScrollableAncestors', () => {
    it('should return an empty list when element is missing', () => {
        expect((0, utils_1.getScrollableAncestors)(null)).toEqual([]);
    });
    it('should find scrollable ancestors inside a shadow root', () => {
        ...
        expect((0, utils_1.getScrollableAncestors)(iframe)).toEqual([scrollContainer]);
        host.remove();
    });
    it('should ignore ancestors when computed style is unavailable', () => {
        ...
        expect((0, utils_1.getScrollableAncestors)(iframe)).toEqual([]);
        getComputedStyleSpy.mockRestore();
    });
});
describe('getClippingAncestors', () => {
    it('should return an empty list when element is missing', () => {
        expect((0, utils_1.getClippingAncestors)(null)).toEqual([]);
    });
    it('should include scrollable and non-scroll clipping ancestors', () => {
        ...
        expect((0, utils_1.getClippingAncestors)(iframe)).toEqual([clippingContainer, scrollContainer]);
    });
});
describe('querySelectorAcrossShadowRoot', () => {
    it('should resolve a selector from the light DOM document', () => {
        ...
        expect((0, utils_1.querySelectorAcrossShadowRoot)('#light-dom-target')).toBe(el);
        el.remove();
    });
    it('should resolve a selector inside the reference node shadow root when the document misses', () => {
        ...
        expect((0, utils_1.querySelectorAcrossShadowRoot)('#shadow-target', reference)).toBe(container);
        host.remove();
    });
    it('should prefer the document match over the shadow root', () => {
        ...
        shadow.appendChild(reference);
        expect((0, utils_1.querySelectorAcrossShadowRoot)('#shared-id', reference)).toBe(lightEl);
        lightEl.remove();
        host.remove();
    });
    it('should return null when nothing matches and the reference node is in the light DOM', () => {
        ...
        expect((0, utils_1.querySelectorAcrossShadowRoot)('#does-not-exist', reference)).toBeNull();
        reference.remove();
    });
    it('should return null when no reference node is provided and the document misses', () => {
        expect((0, utils_1.querySelectorAcrossShadowRoot)('#does-not-exist')).toBeNull();
    });
});
describe('getEffectiveClippingAncestors', () => {
    it('should return an empty list when element is missing', () => {
        expect((0, utils_1.getEffectiveClippingAncestors)(null)).toEqual([]);
    });
    it('should ignore overflow ancestors that do not clip the element', () => {
        ...
        expect((0, utils_1.getEffectiveClippingAncestors)(iframe)).toEqual([]);
    });
    it('should include overflow ancestors that clip the element', () => {
        ...
        expect((0, utils_1.getEffectiveClippingAncestors)(iframe)).toEqual([clippingContainer]);
    });
});
...
describe('isValidCssMargin', () => {
    it('should return true for valid CSS margin values', () => {
        expect((0, utils_1.isValidCssMargin)('10px')).toBe(true);
        expect((0, utils_1.isValidCssMargin)('0px')).toBe(true);
        expect((0, utils_1.isValidCssMargin)('20%')).toBe(true);
        expect((0, utils_1.isValidCssMargin)('0')).toBe(true);
    });
    it('should return false for invalid CSS margin values', () => {
        expect((0, utils_1.isValidCssMargin)('')).toBe(false);
        expect((0, utils_1.isValidCssMargin)('   ')).toBe(false);
        expect((0, utils_1.isValidCssMargin)('invalid')).toBe(false);
        expect((0, utils_1.isValidCssMargin)('10')).toBe(false); // missing unit
    });
    it('should return false and log error when value is not a string (non-string type)', () => {
        ...
        expect((0, utils_1.isValidCssMargin)(42)).toBe(false);
        expect(logger_1.logger.error).toHaveBeenCalledWith('Please provide a valid lazyLoadingMargin value (e.g., "10px")');
    });
    it('should return false when more than 4 space-separated parts are given', () => {
        // Covers line 157-158: parts.length > 4 branch
        expect((0, utils_1.isValidCssMargin)('10px 20px 30px 40px 50px')).toBe(false);
        expect(logger_1.logger.error).toHaveBeenCalledWith('Please provide a valid lazyLoadingMargin value (e.g., "10px")');
    });
    it('should return true for valid multi-part margin (up to 4 parts)', () => {
        expect((0, utils_1.isValidCssMargin)('10px 20px')).toBe(true);
        expect((0, utils_1.isValidCssMargin)('10px 20px 30px 40px')).toBe(true);
        expect((0, utils_1.isValidCssMargin)('auto')).toBe(true);
    });
});
describe('getValueFromWindow and storeValueInWindow', () => {
    describe('SSR environment handling', () => {
        ...
        test('storeValueInWindow should log error in SSR environment', () => {
            ...
            const result = (0, utils_1.storeValueInWindow)('testKey', 'testValue');
            expect(logger_1.logger.error).toHaveBeenCalledWith(errors_1.ERROR_MESSAGE.SSR_ENVIRONMENT_ERROR);
            expect(result).toBe('testValue');
        });
        test('getValueFromWindow should log error in SSR environment', () => {
            delete global.window;
            const result = (0, utils_1.getValueFromWindow)('testKey');
            expect(logger_1.logger.error).toHaveBeenCalledWith(errors_1.ERROR_MESSAGE.SSR_ENVIRONMENT_ERROR);
            expect(result).toBeUndefined();
        });
        test('resetValueFromWindow should log error in SSR environment', () => {
            delete global.window;
            const result = (0, utils_1.resetValueFromWindow)('testKey');
            expect(logger_1.logger.error).toHaveBeenCalledWith(errors_1.ERROR_MESSAGE.SSR_ENVIRONMENT_ERROR);
    ...
    describe('resetValueFromWindow', () => {
        ...
        test('should reset existing key and return true', () => {
            (0, utils_1.storeValueInWindow)('keyToReset', 'someValue');
            expect((0, utils_1.getValueFromWindow)('keyToReset')).toBe('someValue');
            const result = (0, utils_1.resetValueFromWindow)('keyToReset');
            expect(result).toBe(true);
            expect((0, utils_1.getValueFromWindow)('keyToReset')).toBe(undefined);
        });
        test('should return false when key does not exist', () => {
            const result = (0, utils_1.resetValueFromWindow)('nonExistentKey');
            expect(result).toBe(false);
        });
        test('should only reset the specified key', () => {
            (0, utils_1.storeValueInWindow)('key1', 'value1');
            (0, utils_1.storeValueInWindow)('key2', 'value2');
            (0, utils_1.resetValueFromWindow)('key1');
            expect((0, utils_1.getValueFromWindow)('key1')).toBe(undefined);
            expect((0, utils_1.getValueFromWindow)('key2')).toBe('value2');
        });
    });
    describe('validateHttpUrl', () => {
        test.each([
            ...
            const [isValid, error] = (0, utils_1.validateHttpUrl)(url);
        ...
        test.each([
            ...
            expect(error === null || error === void 0 ? void 0 : error.message).toContain('Invalid protocol');
            expect(error === null || error === void 0 ? void 0 : error.message).toContain(protocol);
        });
        test.each([
    ...
    describe('setParamIfDefined', () => {
        test.each([
            ...
            const queryParams = {};
            (0, utils_1.setParamIfDefined)(queryParams, param, value, asBoolean);
            expect(queryParams[param]).toBe(expected);
        });
        test('should not set param when value is undefined', () => {
            const queryParams = {};
            (0, utils_1.setParamIfDefined)(queryParams, 'testParam', undefined);
...
describe('getSSOMarker', () => {
    test('returns tsSSOMarker query string with encoded markerId', () => {
        expect((0, utils_1.getSSOMarker)('abc123')).toBe('tsSSOMarker=abc123');
    });
    test('URL-encodes special characters in markerId', () => {
        expect((0, utils_1.getSSOMarker)('hello world')).toBe('tsSSOMarker=hello%20world');
        expect((0, utils_1.getSSOMarker)('foo=bar&baz')).toBe('tsSSOMarker=foo%3Dbar%26baz');
    });
    test('handles empty string markerId', () => {
        expect((0, utils_1.getSSOMarker)('')).toBe('tsSSOMarker=');
    });
});
// ---------------------------------------------------------------------------
// deepMerge
...
describe('deepMerge', () => {
    test('merges two flat objects', () => {
        const result = (0, utils_1.deepMerge)({ a: 1 }, { b: 2 });
        expect(result).toEqual({ a: 1, b: 2 });
    });
    test('source overrides target for same keys', () => {
        const result = (0, utils_1.deepMerge)({ a: 1, b: 2 }, { b: 99 });
        expect(result).toEqual({ a: 1, b: 99 });
    });
    test('deep merges nested objects', () => {
        const result = (0, utils_1.deepMerge)({ a: { x: 1, y: 2 } }, { a: { y: 99, z: 3 } });
        expect(result).toEqual({ a: { x: 1, y: 99, z: 3 } });
    });
    test('handles empty source', () => {
        const result = (0, utils_1.deepMerge)({ a: 1 }, {});
        expect(result).toEqual({ a: 1 });
    });
    test('handles empty target', () => {
        const result = (0, utils_1.deepMerge)({}, { b: 2 });
...
describe('getHostEventsConfig', () => {
    test('returns shouldBypassPayloadValidation and useHostEventsV2 from viewConfig', () => {
        const viewConfig = { shouldBypassPayloadValidation: true, useHostEventsV2: false };
        expect((0, utils_1.getHostEventsConfig)(viewConfig)).toEqual({
            shouldBypassPayloadValidation: true,
            useHostEventsV2: false,
        });
    });
    test('returns undefined for missing keys', () => {
        const result = (0, utils_1.getHostEventsConfig)({});
...
describe('isWindowUndefined', () => {
    test('returns false when window is defined (browser/jsdom env)', () => {
        ...
        expect((0, utils_1.isWindowUndefined)()).toBe(false);
    });
});
// ---------------------------------------------------------------------------
// getOffsetTop
...
describe('getOffsetTop', () => {
    test('returns rect.top + window.scrollY', () => {
        ...
        expect((0, utils_1.getOffsetTop)(mockElement)).toBe(150);
    });
    test('returns rect.top when scrollY is 0', () => {
        ...
        expect((0, utils_1.getOffsetTop)(mockElement)).toBe(30);
    });
});
// ---------------------------------------------------------------------------
// getDOMNode
...
describe('getDOMNode', () => {
    test('returns the element from document.querySelector when passed a string selector', () => {
        ...
        expect((0, utils_1.getDOMNode)('#test-dom-node')).toBe(div);
        document.body.removeChild(div);
    });
    test('returns the element directly when passed an HTMLElement', () => {
        const div = document.createElement('div');
        expect((0, utils_1.getDOMNode)(div)).toBe(div);
    });
    test('returns null for a selector that matches nothing', () => {
        expect((0, utils_1.getDOMNode)('#nonexistent-element-xyz')).toBeNull();
    });
});
// ---------------------------------------------------------------------------
...
describe('getOperationNameFromQuery', () => {
    test('extracts operation name from a query string', () => {
        expect((0, utils_1.getOperationNameFromQuery)('query GetUser { user { id } }')).toBe('GetUser');
    });
    test('extracts operation name from a mutation string', () => {
        expect((0, utils_1.getOperationNameFromQuery)('mutation CreateUser { createUser { id } }')).toBe('CreateUser');
    });
    test('returns undefined when no operation name is present', () => {
        expect((0, utils_1.getOperationNameFromQuery)('{ user { id } }')).toBeUndefined();
    });
    test('handles multiline query strings', () => {
        ...
        `;
        expect((0, utils_1.getOperationNameFromQuery)(query)).toBe('FetchData');
    });
});
