...
const auto_frame_renderer_1 = require("./auto-frame-renderer");
...
const TSMCP_SRC = `https://${thoughtSpotHost}/v2/?${types_1.Param.Tsmcp}=true#/embed/viz/lb1`;
describe('startAutoMCPFrameRenderer', () => {
    let renderIFrameSpy;
    let getEmbedBasePathSpy;
    beforeAll(() => {
        (0, index_1.init)({
            thoughtSpotHost,
    ...
    beforeEach(() => {
        ...
        getEmbedBasePathSpy = jest.spyOn(ts_embed_1.TsEmbed.prototype, 'getEmbedBasePath').mockImplementation(function (query) {
            return `http://${thoughtSpotHost}/${query}#`;
        });
        renderIFrameSpy = jest.spyOn(ts_embed_1.TsEmbed.prototype, 'renderIFrame').mockResolvedValue(undefined);
    });
    afterEach(() => {
    ...
    async function captureRenderedSrc(viewConfig = {}) {
        let capturedSrc = '';
        renderIFrameSpy.mockRestore();
        renderIFrameSpy = jest.spyOn(ts_embed_1.TsEmbed.prototype, 'renderIFrame')
            .mockImplementation(async function (src) {
            capturedSrc = src;
        });
        const observer = (0, auto_frame_renderer_1.startAutoMCPFrameRenderer)(viewConfig);
     ...
     */
    async function getAppInitResponse(viewConfig = {}) {
        var _a;
        renderIFrameSpy.mockRestore();
        renderIFrameSpy = jest.spyOn(ts_embed_1.TsEmbed.prototype, 'renderIFrame')
            .mockImplementation(async function (src) {
        ...
        (0, test_utils_1.postMessageToParent)(embeddedIframe.contentWindow, { type: types_1.EmbedEvent.APP_INIT, data: {} }, mockPort);
        await new Promise((r) => setTimeout(r, 50));
        observer.disconnect();
        return (_a = mockPort.postMessage.mock.calls[0]) === null || _a === void 0 ? void 0 : _a[0];
    }
    // ─── MutationObserver setup ───────────────────────────────────────────────
    describe('MutationObserver setup', () => {
        test('should return a MutationObserver', () => {
            const observer = (0, auto_frame_renderer_1.startAutoMCPFrameRenderer)();
    ...
    describe('iframe detection via tsmcp param', () => {
        test('should process directly-added iframes with tsmcp=true', async () => {
            const observer = (0, auto_frame_renderer_1.startAutoMCPFrameRenderer)();
            const iframe = document.createElement('iframe');
            iframe.src = `https://${thoughtSpotHost}/v2/?${types_1.Param.Tsmcp}=true#/embed/viz/lb1/tab1`;
            document.body.appendChild(iframe);
            await new Promise((r) => setTimeout(r, 50));
        ...
        test('should process tsmcp iframes nested inside added elements', async () => {
            ...
            const iframe = document.createElement('iframe');
            iframe.src = `https://${thoughtSpotHost}/?${types_1.Param.Tsmcp}=true`;
        ...
        test('should ignore non-iframe element nodes', async () => {
            ...
            div.textContent = `${types_1.Param.Tsmcp}=true`;
        ...
        test('should process multiple tsmcp iframes in one mutation', async () => {
            ...
            iframe1.src = `https://${thoughtSpotHost}/?${types_1.Param.Tsmcp}=true&id=1`;
            const iframe2 = document.createElement('iframe');
            iframe2.src = `https://${thoughtSpotHost}/?${types_1.Param.Tsmcp}=true&id=2`;
    ...
    describe('URL params forwarding', () => {
        ...
        test('disabledActions → disableAction in rendered src', async () => {
            const src = await captureRenderedSrc({ disabledActions: [types_1.Action.Pin] });
        ...
        test('disabledActionReason → disableHint in rendered src', async () => {
            const src = await captureRenderedSrc({
                disabledActions: [types_1.Action.Pin],
        ...
        test('hiddenActions → hideAction in rendered src as JSON array', async () => {
            const src = await captureRenderedSrc({ hiddenActions: [types_1.Action.Pin] });
            expect(src).toContain(`hideAction=${JSON.stringify([types_1.Action.ReportError, types_1.Action.Pin])}`);
        });
        test('visibleActions → visibleAction in rendered src', async () => {
            const src = await captureRenderedSrc({ visibleActions: [types_1.Action.Download] });
        ...
        test('multiple disabledActions → all actions serialised in rendered src', async () => {
            const src = await captureRenderedSrc({
                disabledActions: [types_1.Action.Pin, types_1.Action.Download, types_1.Action.Save],
            });
            expect(src).toContain('disableAction');
            expect(src).toContain(types_1.Action.Pin);
            expect(src).toContain(types_1.Action.Download);
            expect(src).toContain(types_1.Action.Save);
        });
        test('multiple hiddenActions → all actions serialised in rendered src', async () => {
            var _a, _b;
            const src = await captureRenderedSrc({
                hiddenActions: [types_1.Action.Pin, types_1.Action.Download],
            });
            const hideParam = decodeURIComponent((_b = (_a = src.split('hideAction=')[1]) === null || _a === void 0 ? void 0 : _a.split('&')[0]) !== null && _b !== void 0 ? _b : '');
            const parsed = JSON.parse(hideParam);
            expect(parsed).toContain(types_1.Action.Pin);
            expect(parsed).toContain(types_1.Action.Download);
        });
        test('multiple visibleActions → all actions serialised in rendered src', async () => {
            var _a, _b;
            const src = await captureRenderedSrc({
                visibleActions: [types_1.Action.Download, types_1.Action.Save, types_1.Action.Pin],
            });
            const visibleParam = decodeURIComponent((_b = (_a = src.split('visibleAction=')[1]) === null || _a === void 0 ? void 0 : _a.split('&')[0]) !== null && _b !== void 0 ? _b : '');
            const parsed = JSON.parse(visibleParam);
            expect(parsed).toContain(types_1.Action.Download);
            expect(parsed).toContain(types_1.Action.Save);
            expect(parsed).toContain(types_1.Action.Pin);
        });
    ...
    describe('frameParams forwarding', () => {
        test('frameParams.height and .width applied to the replacement iframe element', async () => {
            ...
            const createIframeElSpy = jest.spyOn(ts_embed_1.TsEmbed.prototype, 'createIframeEl');
            const observer = (0, auto_frame_renderer_1.startAutoMCPFrameRenderer)({ frameParams: { height: '600px', width: '100%' } });
            const iframe = document.createElement('iframe');
            iframe.src = TSMCP_SRC;
            document.body.appendChild(iframe);
            await new Promise((r) => setTimeout(r, 50));
            if (createIframeElSpy.mock.calls.length > 0) {
                const created = (_a = createIframeElSpy.mock.results[0]) === null || _a === void 0 ? void 0 : _a.value;
                expect((_b = created === null || created === void 0 ? void 0 : created.style) === null || _b === void 0 ? void 0 : _b.height).toBe('600px');
                expect((_c = created === null || created === void 0 ? void 0 : created.style) === null || _c === void 0 ? void 0 : _c.width).toBe('100%');
            }
        ...
        test('frameParams custom HTML attributes applied to the iframe element', async () => {
            ...
            const createIframeElSpy = jest.spyOn(ts_embed_1.TsEmbed.prototype, 'createIframeEl');
            const observer = (0, auto_frame_renderer_1.startAutoMCPFrameRenderer)({
                frameParams: { height: '400px', width: '800px', 'data-testid': 'my-embed' },
            });
            const iframe = document.createElement('iframe');
            iframe.src = TSMCP_SRC;
            document.body.appendChild(iframe);
            ...
            if (createIframeElSpy.mock.calls.length > 0) {
                const created = (_a = createIframeElSpy.mock.results[0]) === null || _a === void 0 ? void 0 : _a.value;
                expect(created === null || created === void 0 ? void 0 : created.getAttribute('data-testid')).toBe('my-embed');
            }
            createIframeElSpy.mockRestore();
            observer.disconnect();
        });
    ...
    describe('APP_INIT params forwarding', () => {
        test('customizations.style.customCSS included in APP_INIT response', async () => {
            ...
            expect((_c = (_b = (_a = response === null || response === void 0 ? void 0 : response.data) === null || _a === void 0 ? void 0 : _a.customisations) === null || _b === void 0 ? void 0 : _b.style) === null || _c === void 0 ? void 0 : _c.customCSS).toEqual(customizations.style.customCSS);
        });
        test('customizations.content.strings included in APP_INIT response', async () => {
            ...
            const response = await getAppInitResponse({ customizations });
            expect((_d = (_c = (_b = (_a = response === null || response === void 0 ? void 0 : response.data) === null || _a === void 0 ? void 0 : _a.customisations) === null || _b === void 0 ? void 0 : _b.content) === null || _c === void 0 ? void 0 : _c.strings) === null || _d === void 0 ? void 0 : _d.DATA).toBe('Data');
        });
        test('customActions included in APP_INIT response', async () => {
            ...
            expect((_a = response === null || response === void 0 ? void 0 : response.data) === null || _a === void 0 ? void 0 : _a.customActions).toEqual(expect.arrayContaining([expect.objectContaining({ id: 'my-action' })]));
        });
        test('shouldBypassPayloadValidation forwarded via APP_INIT', async () => {
            ...
            expect((_a = response === null || response === void 0 ? void 0 : response.data) === null || _a === void 0 ? void 0 : _a.shouldBypassPayloadValidation).toBe(true);
        });
        test('useHostEventsV2 forwarded via APP_INIT', async () => {
            var _a;
            const response = await getAppInitResponse({ useHostEventsV2: true });
            expect((_a = response === null || response === void 0 ? void 0 : response.data) === null || _a === void 0 ? void 0 : _a.useHostEventsV2).toBe(true);
        });
        test('refreshAuthTokenOnNearExpiry forwarded as embedExpiryInAuthToken', async () => {
            ...
            expect((_a = response === null || response === void 0 ? void 0 : response.data) === null || _a === void 0 ? void 0 : _a.embedExpiryInAuthToken).toBe(true);
        });
        test('interceptUrls forwarded via APP_INIT', async () => {
            ...
            expect((_a = response === null || response === void 0 ? void 0 : response.data) === null || _a === void 0 ? void 0 : _a.interceptUrls).toBeInstanceOf(Array);
            expect((_b = response === null || response === void 0 ? void 0 : response.data) === null || _b === void 0 ? void 0 : _b.interceptUrls.length).toBeGreaterThan(0);
        });
        test('interceptTimeout forwarded via APP_INIT', async () => {
            var _a;
            const response = await getAppInitResponse({ interceptTimeout: 5000 });
            expect((_a = response === null || response === void 0 ? void 0 : response.data) === null || _a === void 0 ? void 0 : _a.interceptTimeout).toBe(5000);
        });
    });
    ...
    describe('getMCPIframeSrc URL construction', () => {
        test('should strip tsmcp param and merge embed params into rendered src', async () => {
            ...
            const observer = (0, auto_frame_renderer_1.startAutoMCPFrameRenderer)();
            const iframe = document.createElement('iframe');
            iframe.src = `https://${thoughtSpotHost}/v2/?${types_1.Param.Tsmcp}=true&customParam=hello#/embed/viz`;
            document.body.appendChild(iframe);
            await new Promise((r) => setTimeout(r, 50));
            expect(capturedSrc).not.toContain(`${types_1.Param.Tsmcp}=true`);
        ...
        test('should preserve hash from original iframe src', async () => {
            ...
            const observer = (0, auto_frame_renderer_1.startAutoMCPFrameRenderer)();
            const iframe = document.createElement('iframe');
            iframe.src = `https://${thoughtSpotHost}/v2/?${types_1.Param.Tsmcp}=true#/embed/viz/lb123`;
            document.body.appendChild(iframe);
            await new Promise((r) => setTimeout(r, 50));
        ...
        test('source params take precedence over viewConfig params for the same key', async () => {
            ...
            const observer = (0, auto_frame_renderer_1.startAutoMCPFrameRenderer)({ additionalFlags: { locale: 'en-US' } });
            const iframe = document.createElement('iframe');
            // Source overrides with de-DE
            iframe.src = `https://${thoughtSpotHost}/v2/?${types_1.Param.Tsmcp}=true&locale=de-DE#/embed/viz`;
            document.body.appendChild(iframe);
            await new Promise((r) => setTimeout(r, 50));
    ...
    describe('URL serialization parity with LiveboardEmbed', () => {
        async function captureLiveboardSrc(viewConfig) {
            let capturedSrc = '';
            renderIFrameSpy.mockRestore();
            renderIFrameSpy = jest.spyOn(ts_embed_1.TsEmbed.prototype, 'renderIFrame')
                .mockImplementation(async function (src) {
        ...
        test.each([
            ['hiddenActions', { hiddenActions: [types_1.Action.Pin] }, 'hideAction'],
            ['disabledActions', { disabledActions: [types_1.Action.Pin] }, 'disableAction'],
            ['visibleActions', { visibleActions: [types_1.Action.Download] }, 'visibleAction'],
        ])('%s: auto-renderer and LiveboardEmbed produce identical param format', async (_, viewConfig, paramName) => {
        ...
        test('hideAction value is a JSON array, not CSV', async () => {
            const autoSrc = await captureRenderedSrc({ hiddenActions: [types_1.Action.Pin] });
            const liveboardSrc = await captureLiveboardSrc({ hiddenActions: [types_1.Action.Pin] });
...
//# sourceMappingURL=auto-frame-renderer.spec.js.map