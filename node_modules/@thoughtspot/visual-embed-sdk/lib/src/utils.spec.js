import { getQueryParamString, getFilterQuery, getCssDimension, getEncodedQueryParamsString, appendToUrlHash, getRedirectUrl, checkReleaseVersionInBeta, getRuntimeParameters, removeTypename, removeStyleProperties, setStyleProperties, isUndefined, storeValueInWindow, getValueFromWindow, handlePresentEvent, handleExitPresentMode, getTypeFromValue, arrayIncludesString, calculateVisibleElementData, getClippingAncestors, getEffectiveClippingAncestors, getScrollableAncestors, formatTemplate, isValidCssMargin, resetValueFromWindow, validateHttpUrl, setParamIfDefined, querySelectorAcrossShadowRoot, getSSOMarker, deepMerge, getHostEventsConfig, isWindowUndefined, getOffsetTop, getDOMNode, getOperationNameFromQuery, } from './utils';
...
describe('unit test for utils', () => {
    ...
    test('getParameterOverride', () => {
        ...
        expect(getRuntimeParameters(params)).toBe('param1=foo&paramVal1=42&param2=bar&paramVal2=abc&param3=baz&paramVal3=true');
    });
    ...
    describe('getRedirectURL', () => {
        ...
        test('Should return correct value when path is set', () => {
            expect(getRedirectUrl('http://myhost:3000/', 'hashFrag', '/bar')).toBe('http://localhost/bar#?tsSSOMarker=hashFrag');
            expect(getRedirectUrl('http://myhost:3000/#/foo', 'hashFrag', '#/bar')).toBe('http://localhost/#/bar?tsSSOMarker=hashFrag');
        });
    });
...
describe('Fullscreen Utility Functions', () => {
    ...
    describe('handlePresentEvent', () => {
        ...
        it('should catch error and log warning when requestFullscreen throws (covers line 506)', async () => {
            ...
            expect(logger.warn).toHaveBeenCalledWith(expect.stringContaining('Failed to enter fullscreen'), expect.any(Error));
        });
...
describe('calculateVisibleElementData', () => {
    ...
    it('should return zero dimensions when element is missing', () => {
        expect(calculateVisibleElementData(null)).toEqual({
            top: 0,
            height: 0,
            left: 0,
            width: 0,
...
describe('getScrollableAncestors', () => {
    it('should return an empty list when element is missing', () => {
        expect(getScrollableAncestors(null)).toEqual([]);
    });
...
describe('getClippingAncestors', () => {
    it('should return an empty list when element is missing', () => {
        expect(getClippingAncestors(null)).toEqual([]);
    });
...
describe('getEffectiveClippingAncestors', () => {
    it('should return an empty list when element is missing', () => {
        expect(getEffectiveClippingAncestors(null)).toEqual([]);
    });
...
describe('formatTemplate', () => {
    it('should replace placeholders with provided values', () => {
        expect(formatTemplate('Hello {name}, you are {age} years old', { name: 'John', age: 30 })).toBe('Hello John, you are 30 years old');
        expect(formatTemplate('Expected {type}, but received {actual}', {
            type: 'string',
            actual: 'number',
        })).toBe('Expected string, but received number');
        expect(formatTemplate('Hello {name}, you are {age} years old', { name: 'John' })).toBe('Hello John, you are {age} years old');
    });
});
describe('isValidCssMargin', () => {
    ...
    it('should return false and log error when value is not a string (non-string type)', () => {
        ...
        expect(isValidCssMargin(42)).toBe(false);
...
describe('getHostEventsConfig', () => {
    ...
    test('returns undefined for missing keys', () => {
        const result = getHostEventsConfig({});
