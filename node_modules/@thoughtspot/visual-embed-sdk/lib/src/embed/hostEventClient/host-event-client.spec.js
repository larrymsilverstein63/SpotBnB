...
describe('HostEventClient', () => {
    ...
    beforeEach(() => {
        jest.spyOn(EmbedConfigService, 'getEmbedConfig').mockReturnValue({ thoughtSpotHost: mockThoughtSpotHost, authType: AuthType.None });
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
    describe('executeUIPassthroughApi', () => {
        it('should call processTrigger with correct parameters and return response', async () => {
            ...
            expect(mockProcessTrigger).toHaveBeenCalledWith(mockIframe, HostEvent.UIPassthrough, mockThoughtSpotHost, {
                type: apiName,
                parameters,
            }, undefined);
    ...
    describe('executeHostEvent', () => {
        ...
        it('should call handleUIPassthroughForHostEvent for Pin event', async () => {
            ...
            expect(mockProcessTrigger).toHaveBeenCalledWith(mockIframe, HostEvent.UIPassthrough, 'http://localhost', { parameters: payload, type: UIPassthroughEvent.PinAnswerToLiveboard }, undefined);
        ...
        it('should accept UpdateFilters with filters array', async () => {
            ...
            expect(mockProcessTrigger).toHaveBeenNthCalledWith(2, mockIframe, HostEvent.UIPassthrough, mockThoughtSpotHost, { type: UIPassthroughEvent.UpdateFilters, parameters: payload }, undefined);
            expect(result).toEqual({ success: true });
        });
        ...
        it('should pass context to UpdateFilters event', async () => {
            ...
            expect(mockProcessTrigger).toHaveBeenCalledWith(mockIframe, HostEvent.UIPassthrough, mockThoughtSpotHost, { type: UIPassthroughEvent.UpdateFilters, parameters: payload }, context);
        });
        it('should dispatch UpdateParameters over the legacy channel', async () => {
            ...
            expect(mockProcessTrigger).toHaveBeenCalledTimes(1);
            expect(mockProcessTrigger).toHaveBeenCalledWith(mockIframe, HostEvent.UpdateParameters, mockThoughtSpotHost, payload, undefined);
            expect(result).toEqual({ success: true });
        });
        it('should forward UpdateParameters with null applicability', async () => {
            const { client, mockIframe } = createHostEventClient();
            const payload = [{ name: 'param1', value: 10, applicability: null }];
            mockProcessTrigger.mockResolvedValueOnce({ success: true });
        ...
        it('should pass context to UpdateParameters event', async () => {
            const { client, mockIframe } = createHostEventClient();
            const payload = [{ name: 'param1', value: 10 }];
            const context = { vizId: 'viz-1' };
            mockProcessTrigger.mockResolvedValueOnce({ success: true });
            await client.triggerHostEvent(HostEvent.UpdateParameters, payload, context);
            expect(mockProcessTrigger).toHaveBeenCalledWith(mockIframe, HostEvent.UpdateParameters, mockThoughtSpotHost, payload, context);
        });
        ...
        it('should pass context to DrillDown event', async () => {
            ...
            expect(mockProcessTrigger).toHaveBeenCalledWith(mockIframe, HostEvent.UIPassthrough, mockThoughtSpotHost, { type: UIPassthroughEvent.Drilldown, parameters: payload }, context);
        });
        it('should skip to fallback when passthrough is not in available keys', async () => {
            ...
            expect(mockProcessTrigger).toHaveBeenNthCalledWith(2, mockIframe, HostEvent.GetAnswerSession, mockThoughtSpotHost, { vizId: '123' }, undefined);
        ...
        it('should route GetAnswerSession through passthrough and return data', async () => {
            ...
            mockProcessTrigger
                ...
                .mockResolvedValueOnce(mockResponse);
            const result = await client.triggerHostEvent(HostEvent.GetAnswerSession, { vizId: '123' });
            expect(mockProcessTrigger).toHaveBeenNthCalledWith(2, mockIframe, HostEvent.UIPassthrough, mockThoughtSpotHost, { type: UIPassthroughEvent.GetAnswerSession, parameters: { vizId: '123' } }, undefined);
        ...
        it('should fall back to legacy host event when passthrough returns no data for GetAnswerSession', async () => {
            ...
            expect(mockProcessTrigger).toHaveBeenNthCalledWith(3, expect.anything(), HostEvent.GetAnswerSession, mockThoughtSpotHost, { vizId: '123' }, undefined);
        ...
        it('should route GetFilters through passthrough and return data', async () => {
            ...
            expect(mockProcessTrigger).toHaveBeenCalledWith(mockIframe, HostEvent.UIPassthrough, mockThoughtSpotHost, { type: UIPassthroughEvent.GetFilters, parameters: {} }, undefined);
        ...
        it('should call fallback  for Pin event', async () => {
            ...
            mockProcessTrigger
                ...
                .mockResolvedValueOnce([mockResponse]);
            const result = await client.triggerHostEvent(hostEvent, payload);
            expect(mockProcessTrigger).toHaveBeenCalledWith(mockIframe, HostEvent.Pin, mockThoughtSpotHost, {}, undefined);
            expect(result).toEqual([mockResponse]);
        });
        it('should call fallback for SaveAnswer event', async () => {
            ...
            mockProcessTrigger
                ...
                .mockResolvedValueOnce([mockResponse]);
            const result = await client.triggerHostEvent(hostEvent, payload);
            expect(mockProcessTrigger).toHaveBeenCalledWith(mockIframe, HostEvent.Save, mockThoughtSpotHost, {}, undefined);
            expect(result).toEqual([mockResponse]);
        });
    ...
    describe('getDataWithPassthroughFallback', () => {
        ...
        it('should return unwrapped value when passthrough succeeds', async () => {
            ...
            const result = await callMethod(client, UIPassthroughEvent.GetAnswerSession, HostEvent.GetAnswerSession, { vizId: '1' });
        ...
        it('should fall back to legacy when passthrough returns null', async () => {
            ...
            mockProcessTrigger
                .mockResolvedValueOnce(null)
                .mockResolvedValueOnce(legacyResponse);
            const result = await callMethod(client, UIPassthroughEvent.GetParameters, HostEvent.GetParameters, {});
            expect(mockProcessTrigger).toHaveBeenCalledTimes(2);
            expect(result).toEqual(legacyResponse);
        });
        it('should fall back to legacy when passthrough returns undefined', async () => {
            ...
            mockProcessTrigger
                ...
                .mockResolvedValueOnce(legacyResponse);
            const result = await callMethod(client, UIPassthroughEvent.GetIframeUrl, HostEvent.GetIframeUrl, {});
            expect(mockProcessTrigger).toHaveBeenCalledTimes(2);
            expect(result).toEqual(legacyResponse);
        });
        it('should fall back when response array has no matching entries', async () => {
            ...
            mockProcessTrigger
                ...
                .mockResolvedValueOnce(legacyResponse);
            const result = await callMethod(client, UIPassthroughEvent.GetExportRequestForCurrentPinboard, HostEvent.getExportRequestForCurrentPinboard, {});
            expect(mockProcessTrigger).toHaveBeenCalledTimes(2);
            expect(result).toEqual(legacyResponse);
        });
        it('should throw when response has error field', async () => {
            ...
            await expect(callMethod(client, UIPassthroughEvent.GetFilters, HostEvent.GetFilters, {})).rejects.toThrow('Permission denied');
            expect(mockProcessTrigger).toHaveBeenCalledTimes(1);
        });
        it('should throw when response value contains errors field', async () => {
            ...
            await expect(callMethod(client, UIPassthroughEvent.GetTML, HostEvent.GetTML, { vizId: 'bad' })).rejects.toThrow('Invalid vizId');
            expect(mockProcessTrigger).toHaveBeenCalledTimes(1);
        });
        it('should throw when response value contains error field', async () => {
            const { client } = createHostEventClient();
            mockProcessTrigger.mockResolvedValue([{ value: { error: 'Not found' } }]);
            await expect(callMethod(client, UIPassthroughEvent.GetTabs, HostEvent.GetTabs, {})).rejects.toThrow('Not found');
            expect(mockProcessTrigger).toHaveBeenCalledTimes(1);
        });
        it('should stringify object errors instead of producing [object Object]', async () => {
            ...
            await expect(callMethod(client, UIPassthroughEvent.GetFilters, HostEvent.GetFilters, {})).rejects.toThrow(JSON.stringify(errorObj));
            expect(mockProcessTrigger).toHaveBeenCalledTimes(1);
        });
        it('should default payload to empty object when null', async () => {
            ...
            await callMethod(client, UIPassthroughEvent.GetIframeUrl, HostEvent.GetIframeUrl, null);
            expect(mockProcessTrigger).toHaveBeenCalledWith(mockIframe, HostEvent.UIPassthrough, mockThoughtSpotHost, { type: UIPassthroughEvent.GetIframeUrl, parameters: {} }, undefined);
        });
    });
    ...
    describe('availablePassthroughKeysCache reuse', () => {
        ...
        it('GetFilters: second call reuses cache — GetAvailableUIPassthroughs called only once', async () => {
            const { client } = createHostEventClient();
            const filterResponse = [{ value: { liveboardFilters: [], runtimeFilters: [] } }];
            mockProcessTrigger
            ...
            expect(mockProcessTrigger).toHaveBeenCalledTimes(3);
            expect(mockProcessTrigger).not.toHaveBeenNthCalledWith(3, expect.anything(), HostEvent.UIPassthrough, mockThoughtSpotHost, expect.objectContaining({ type: UIPassthroughEvent.GetAvailableUIPassthroughs }), undefined);
        });
        it('GetTabs: second call reuses cache — GetAvailableUIPassthroughs called only once', async () => {
            const { client } = createHostEventClient();
            const tabResponse = [{ value: { orderedTabIds: [], numberOfTabs: 0, Tabs: [] } }];
            mockProcessTrigger
                .mockResolvedValueOnce(allKeys())
        ...
        it('GetParameters: second call reuses cache — GetAvailableUIPassthroughs called only once', async () => {
            const { client } = createHostEventClient();
            const paramResponse = [{ value: { parameters: [] } }];
            mockProcessTrigger
                .mockResolvedValueOnce(allKeys())
        ...
        it('UIPassthrough direct trigger does not populate the cache', async () => {
            ...
            mockProcessTrigger.mockResolvedValueOnce([{ value: { liveboardFilters: [], runtimeFilters: [] } }]);
            await client.triggerHostEvent(HostEvent.GetFilters, {});
        ...
        it('GetAnswerSession: catch in getAvailableUIPassthroughKeys does not cache, so next call retries GetAvailableUIPassthroughs', async () => {
            ...
            const availableCallsCount = mockProcessTrigger.mock.calls.filter((call) => { var _a; return ((_a = call[3]) === null || _a === void 0 ? void 0 : _a.type) === UIPassthroughEvent.GetAvailableUIPassthroughs; }).length;
    ...
    describe('GetTML edge cases', () => {
        ...
        it('includeNonExecutedSearchTokens flag is forwarded in UIPassthrough parameters', async () => {
            ...
            await client.triggerHostEvent(HostEvent.GetTML, { includeNonExecutedSearchTokens: true });
            expect(mockProcessTrigger).toHaveBeenNthCalledWith(2, mockIframe, HostEvent.UIPassthrough, mockThoughtSpotHost, {
        ...
        it('GetTML key absent from non-empty keys array falls back to legacy hostEventFallback', async () => {
            ...
            const result = await client.triggerHostEvent(HostEvent.GetTML, {});
            expect(mockProcessTrigger).toHaveBeenNthCalledWith(2, mockIframe, HostEvent.GetTML, mockThoughtSpotHost, {}, undefined);
    ...
    describe('SaveAnswer key-presence edge cases', () => {
        ...
        it('name+description both empty string route through passthrough, not Save fallback', async () => {
            ...
            await client.triggerHostEvent(HostEvent.SaveAnswer, { name: '', description: '' });
            expect(mockProcessTrigger).toHaveBeenNthCalledWith(2, mockIframe, HostEvent.UIPassthrough, mockThoughtSpotHost, { type: UIPassthroughEvent.SaveAnswer, parameters: { name: '', description: '' } }, undefined);
        });
        it('description: undefined (key present) routes through passthrough, not Save fallback', async () => {
            ...
            mockProcessTrigger
                .mockResolvedValueOnce(allKeys())
                .mockResolvedValueOnce(saveResponse);
            await client.triggerHostEvent(HostEvent.SaveAnswer, { name: 'Test', description: undefined });
            expect(mockProcessTrigger).toHaveBeenNthCalledWith(2, mockIframe, HostEvent.UIPassthrough, mockThoughtSpotHost, { type: UIPassthroughEvent.SaveAnswer, parameters: { name: 'Test', description: undefined } }, undefined);
        });
        it('name key absent falls back to HostEvent.Save', async () => {
            ...
            await client.triggerHostEvent(HostEvent.SaveAnswer, { description: 'desc only' });
            expect(mockProcessTrigger).toHaveBeenCalledWith(mockIframe, HostEvent.Save, mockThoughtSpotHost, { description: 'desc only' }, undefined);
        });
        it('answerId is undefined when saveResponse.data.Answer__save.answer.id is missing', async () => {
            ...
            const result = await client.triggerHostEvent(HostEvent.SaveAnswer, { name: 'Test', description: 'Desc' });
