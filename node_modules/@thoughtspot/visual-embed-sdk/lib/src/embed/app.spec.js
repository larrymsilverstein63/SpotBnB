...
describe('App embed tests', () => {
    ...
    test('should set newConnectionsExperience to true in url', async () => {
        ...
        appEmbed.render();
        await executeAfterWait(() => {
            expectUrlMatchesWithParams(getIFrameSrc(), `http://${thoughtSpotHost}/?embedApp=true&primaryNavHidden=true&profileAndHelpInNavBarHidden=false&newConnectionsExperience=true${defaultParams}${defaultParamsPost}#/home`);
        });
    });
    test('should set newConnectionsExperience to false in url', async () => {
        ...
        appEmbed.render();
        await executeAfterWait(() => {
            expectUrlMatchesWithParams(getIFrameSrc(), `http://${thoughtSpotHost}/?embedApp=true&primaryNavHidden=true&profileAndHelpInNavBarHidden=false&newConnectionsExperience=false${defaultParams}${defaultParamsPost}#/home`);
        });
    });
    ...
    test('should register event handlers to adjust iframe height', async () => {
        ...
        const onSpy = jest.spyOn(AppEmbed.prototype, 'on').mockImplementation((event, callback) => {
            if (event === EmbedEvent.RouteChange) {
                callback({ type: EmbedEvent.RouteChange, data: { currentPath: '/answers' } }, jest.fn());
            }
            if (event === EmbedEvent.EmbedHeight) {
                embedHeightCallback = callback;
            }
            if (event === EmbedEvent.EmbedIframeCenter) {
                callback({ type: EmbedEvent.EmbedIframeCenter, data: {} }, jest.fn());
            }
            return null;
        });
    ...
    describe('Navigate to Page API', () => {
        ...
        test('Do not allow number path without noReload in navigateToPage', async () => {
            ...
            expect(logger.warn).toHaveBeenCalledWith('Path can only by a string when triggered without noReload');
        });
...
describe('AppEmbed uncovered branch tests', () => {
    ...
    test('registerLazyLoadEvents should return early when iFrame is not set', () => {
        ...
        expect(() => {
            appEmbed.registerLazyLoadEvents();
        }).not.toThrow();
    });
});
