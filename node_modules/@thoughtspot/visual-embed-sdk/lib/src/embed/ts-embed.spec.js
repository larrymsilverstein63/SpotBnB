...
import { AuthType, init, EmbedEvent, SearchEmbed, PinboardEmbed, AppEmbed, LiveboardEmbed, AnswerService, SpotterEmbed, } from '../index';
...
import { executeAfterWait, getDocumentBody, getIFrameEl, getIFrameSrc, getRootEl, postMessageToParent, defaultParamsForPinboardEmbed, waitFor, expectUrlMatchesWithParams, expectUrlToHaveParamsWithValues, mockMessageChannel, createRootEleForEmbed, fixedEncodeURI, } from '../test/test-utils';
...
describe('Unit test case for ts embed', () => {
    ...
    describe('Trigger infoSuccess event on iframe load', () => {
        ...
        test('should NOT trigger InfoSuccess when getPreauthInfo resolves with no .info field', async () => {
            ...
            const tsEmbed = new SearchEmbed(getRootEl(), {});
            iFrame.contentWindow = { postMessage: jest.fn() };
            jest.spyOn(iFrame, 'addEventListener').mockImplementationOnce((event, handler) => { handler({}); });
            jest.spyOn(document, 'createElement').mockReturnValueOnce(iFrame);
            await tsEmbed.render();
            await executeAfterWait(() => {
                const infoSuccessCall = mockProcessTrigger.mock.calls.find((call) => call[1] === HostEvent.InfoSuccess);
    ...
    describe('V1Embed ', () => {
        ...
        test('when isRendered is true than isError will be true', async () => {
            ...
            expect(logger.warn).toHaveBeenCalledWith('Please register event handlers before calling render');
        });
    ...
    describe('Navigate to Page API', () => {
        ...
        test('when app is PinboardEmbed after navigateToPage function call, new path should be set to iframe', async () => {
            ...
            expectUrlMatchesWithParams(getIFrameSrc(), `http://${thoughtSpotHost}/?embedApp=true&${defaultParamsForPinboardEmbed}&isLiveboardEmbed=true${defaultParamsPost}#/embed/${path}`);
        });
    ...
    describe('Iframe flags', () => {
        ...
        it('Should set the override locale for number/date and currency format', async () => {
            ...
            await appEmbed.render();
            expectUrlMatchesWithParams(getIFrameSrc(), `http://${thoughtSpotHost}/?embedApp=true&primaryNavHidden=true&profileAndHelpInNavBarHidden=false&numberFormatLocale=en-US&dateFormatLocale=en-IN&currencyFormat=USD${defaultParamsPost}#/home`);
        });
    ...
    describe('validate preRender flow', () => {
        ...
        describe('preRenderContainer', () => {
            ...
            it('should fall back to document.body and log an error when the CSS selector is invalid', async () => {
                ...
                const preRenderWrapper = document.getElementById(preRenderIds.wrapper);
                expect(document.body.contains(preRenderWrapper)).toBe(true);
                expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('Invalid CSS selector for preRenderContainer'), expect.any(Error));
                logger.error.mockClear();
            });
            ...
            it('should restore the original container position on destroy', async () => {
                ...
                expect(libEmbed.preRenderContainerEl).toBeNull();
                customContainer.remove();
            });
...
describe('Trigger method edge cases', () => {
    ...
    test('isValidationError catch: handleError called with embedErrorDetails and error is rethrown', async () => {
        const searchEmbed = new SearchEmbed(getRootEl(), defaultViewConfig);
        const handleErrorSpy = jest.spyOn(searchEmbed, 'handleError').mockImplementation(() => { });
        await searchEmbed.render();
        ...
        await expect(searchEmbed.trigger(HostEvent.DrillDown, {})).rejects.toThrow('DrillDown requires a valid points object');
...
describe('Fullscreen change handler behavior', () => {
    ...
    test('fullscreenchange fires but iFrame.contentWindow is null — trigger NOT called, debug log emitted', () => {
        ...
        embed.iFrame = { contentWindow: null };
        const addEventListenerSpy = jest.spyOn(document, 'addEventListener');
        const loggerDebugSpy = jest.spyOn(logger, 'debug').mockImplementation(() => { });
        const triggerSpy = jest.spyOn(embed, 'trigger').mockImplementation(() => Promise.resolve(null));
        embed['setupFullscreenChangeHandler']();
        ...
        const handler = fullscreenCall[1];
...
describe('ShowPreRender with UpdateEmbedParams', () => {
    ...
    describe('shouldSkipEvent', () => {
        ...
        test('skips Error event when errorType is resolved from data.data.code (nested format)', () => {
            ...
            const embed = makeEmbed({ useHostEventsV2: true, shouldBypassPayloadValidation: true });
            embed.on(EmbedEvent.Error, errorHandler);
            embed.executeCallbacks(EmbedEvent.Error, makeNestedValidationData('nested error'));
            expect(errorHandler).not.toHaveBeenCalled();
        ...
        test('skips Error event when errorType is resolved from data.errorType (flat format)', () => {
            ...
            const embed = makeEmbed({ useHostEventsV2: true, shouldBypassPayloadValidation: true });
            embed.on(EmbedEvent.Error, errorHandler);
            embed.executeCallbacks(EmbedEvent.Error, makeFlatValidationData());
            expect(errorHandler).not.toHaveBeenCalled();
        });
    ...
    describe('showPreRender inserts placeholder into hostElement (SCAL-315058)', () => {
        ...
        it('showPreRender removes a placeholder nested below a direct child without throwing', async () => {
            ...
            await embed.preRender();
            await waitFor(() => !!document.querySelector('#tsEmbed-pre-render-child-ph-nested-test'));
            const hostElement = document.querySelector('#tsEmbedDiv');
            ...
            const currentPlaceholder = embed.getPreRenderPlaceHolderElement();
